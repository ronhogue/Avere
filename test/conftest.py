...
def mnt_nodes(ssh_con, test_vars):
    check = run_ssh_command(
        ssh_con, "ls ~/STATUS.NODES_MOUNTED", ignore_nonzero_rc=True
    )
    if check["rc"]:  # nodes were not already mounted
        ...
        for i, vs_ip in enumerate(test_vars["cluster_vs_ips"]):
            ...
            fstab_line = (
                "{}:/msazure /nfs/node{} nfs ".format(vs_ip, i)
...
def list_locations(test_vars):
    log = logging.getLogger("list_locations")
    locations = test_vars["atd_obj"].list_locations()
    log.info("Resource Group: {}".format(locations))
    return locations
...
def storage_account(test_vars):
    ...
    sa = atd.st_client.storage_accounts.get_properties(
        atd.resource_group, atd.storage_account
    )
...
def ssh_con(test_vars):
    client = create_ssh_client(test_vars["controller_user"], test_vars["controller_ip"])
    yield client
    client.close()
...
def test_vars():
    ...
    vars = {}
    if "VFXT_TEST_VARS_FILE" in os.environ and os.path.isfile(
        os.environ["VFXT_TEST_VARS_FILE"]
    ):
        log.debug(
            "Loading into vars from {} (VFXT_TEST_VARS_FILE)".format(
                os.environ["VFXT_TEST_VARS_FILE"]
            )
        )
    ...
    log.debug(
        "Loaded the following JSON into vars: {}".format(
    ...
    if "VFXT_TEST_VARS_FILE" in os.environ:
        ...
        log.debug(
            "Saving vars to {} (VFXT_TEST_VARS_FILE)".format(
                os.environ["VFXT_TEST_VARS_FILE"]
            )
        )
